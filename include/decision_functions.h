...
namespace stardec {
    ...
    std::vector<std::shared_ptr<leafnode<double>>>::const_iterator max(std::shared_ptr<leafnode<double>> node, double discount_factor) {
        auto it = std::max_element(node->children.cbegin(), node->children.cend(), comp);
        auto maxnode  = *it;
        node->optimal = maxnode;
        node->value   = maxnode->value * discount_factor;
        return it;
    }
    ...
    void maxchance(std::shared_ptr<leafnode<double>> node, double discount_factor) {
    ...
    std::vector<std::shared_ptr<leafnode<double>>>::const_iterator min(std::shared_ptr<leafnode<double>> node, double discount_factor) {
        auto it = std::min_element(node->children.cbegin(), node->children.cend(), comp);
        auto minnode  = *it;
        node->optimal = minnode;
        node->value   = minnode->value * discount_factor;
        return it;
    }
    ...
    void minchance(std::shared_ptr<leafnode<double>> node, double discount_factor) {
    ...
    void maximax(std::shared_ptr<leafnode<double>> node, double discount_factor) {
    ...
    void maximin(std::shared_ptr<leafnode<double>> node, double discount_factor) {
        for(auto child : node->children) {
            for(auto leaves : child->children)
                if(!leaves->is_leaf())
